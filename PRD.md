...
- Special-case buildings (`FIELD`, `WELL`, `TOWN_HALL`) have intentionally
  different placement, storage, worker, and upgrade rules. Do not normalize them
  into generic producer behavior unless tests explicitly demand it.
...
- Game starts immediately in a pygame window; no save/load, menu system, audio, networking, combat, fog of war, zoom, or rotation.
...
- **F-METAL-05 (MUST):** Dev placement tools include tree, stone, and iron. Add new dev tools explicitly; do not overload existing build menu events silently.
...
- Fixed 48px strip. Shows population/housing only, with disk-first population icon. It does not show resource rows or per-cycle income.
...
- **F-UPG-03 (MUST):** Resource-producing levels grant the staffed worker `+5%` movement and `+5%` gathering/processing speed per level above 1, additive with other bonuses, only while assigned to that building.
...
- Level bonus source is `("building_level", building_id)` with value `+0.05 * (level - 1)`. Recompute on reassignment, demolition, and upgrade completion.
