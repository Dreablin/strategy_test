...
- **F-RES-03 (MUST):** Resource flow is physical: resources are stored in building internal storages and Town Hall warehouse; no passive per-cycle wallet income.

### F-TICK — Cycle System

- **F-TICK-01 (MUST):** A cycle lasts exactly **10 seconds** of wall-clock game time (using `pygame.time.get_ticks()`).
- **F-TICK-02 (MUST, revised):** Cycle ticks drive timed systems (worker states, training, construction progress). Production is driven by worker state machines (see **F-PROD-01**), not passive building ticks.
...
- **F-STONE-06 (MUST):** A stonecutter harvests by:
  ...
  5. Depositing `+1 stone` into the building's internal storage; carriers then move it to Town Hall warehouse via transport tasks.
...
(buildings show status/storage and runtime state lines).
```
...
- **F-UPG-03 (MUST, **revised**):** For resource-producing buildings, each level beyond 1 grants the building's *staffed worker* the following permanent additive bonuses:
...
- **F-STORE-04 (MUST):** Each completed gathering cycle deposits `+1` into the building's internal storage. Further movement to other storages is handled by transport logic (carriers), not passive global income.
...
- **F-WORK-13 (SHOULD):** MINER/FARM follow the same active worker-cycle production model as other producer roles (no passive building ticks).
...
### `game.economy` (conceptual)
```python
# Resource flow is warehouse/storage based.
# Producers fill internal storage; carriers transport between buildings.
# Town Hall warehouse is the central persistent storage.
```
