...
class Renderer:
    ...
    @staticmethod
    def draw_gold(surface: pygame.Surface, world: World, camera=None) -> None:
        """Draw world-owned gold veins and buildable ore fragments."""
        ox, oy = Renderer.map_origin(surface, world)
        cam_x, cam_y = (0, 0) if camera is None else camera.offset
        gx_min, gy_min, gx_max, gy_max = Renderer.visible_tile_range(surface, world, camera)
        if gx_max < gx_min or gy_max < gy_min:
        ...
        entries = sorted(
            [
                item
                for item in world.iter_gold_deposits()
                if gx_min <= item[0][0] <= gx_max and gy_min <= item[0][1] <= gy_max
            ],
            key=lambda item: (item[0][0] + item[0][1], item[0][0]),
        )
        for (gx, gy), gold in entries:
            sx, sy = world_to_screen(gx, gy)
            variant = int(getattr(gold, "variant", 0))
            blocking = bool(getattr(gold, "blocking", False))
            try:
                spr = gold_sprite(variant, blocking=blocking)
                anchor_x, anchor_y = gold_sprite_anchor(variant, blocking=blocking)
                off_x, off_y = gold_sprite_offset(variant, blocking=blocking)
            except TypeError:
                spr = gold_sprite()
                anchor_x, anchor_y = spr.get_width() // 2, spr.get_height()
                off_x, off_y = 0, 0
