...
def cow_farm_hide_output_transport_tasks(registry: Any) -> list[TransportTask]:
    """Build low-priority hide export tasks from cow farms to Town Hall warehouse."""
    if registry is None:
        return []
    buildings = list(registry.all())
    town_hall = next((b for b in buildings if b.type_tag == "TOWN_HALL"), None)
    ...
    for building in buildings:
        ...
        if getattr(building, "is_under_construction", False):
            continue
        amount = int(getattr(building, "hide_amount", lambda: 0)())
        if amount <= 0:
            continue
        for _ in range(amount):
            tasks.append(TransportTask(resource="hide", source=building, target=town_hall, priority=0))
    return tasks
...
def winery_output_transport_tasks(registry: Any) -> list[TransportTask]:
    """Build low-priority wine export tasks from Wineries to Town Hall."""
    if registry is None:
        return []
    buildings = list(registry.all())
    town_hall = next((b for b in buildings if b.type_tag == "TOWN_HALL"), None)
    ...
    for building in buildings:
        ...
        if not getattr(building, "active", True):
            continue
        amount = int(building.output_amount())
        if amount <= 0:
            continue
        for _ in range(amount):
