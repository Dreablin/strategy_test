...
class ChickenFarmPanel:
    ...
    @staticmethod
    def storage_lines(farm: ChickenFarm) -> tuple[str, str, str]:
        return (
            flow_line(
                role_key="ui.panel.input",
                resource_key="wheat",
                amount=farm.input_amount(),
                capacity=farm.input_capacity(),
            ),
            flow_line(
                role_key="ui.panel.input",
                resource_key="water",
                amount=farm.water_amount(),
                capacity=farm.water_capacity(),
            ),
            flow_line(
                role_key="ui.panel.output",
                resource_key="chicken",
                amount=farm.output_amount(),
                capacity=farm.output_capacity(),
            ),
        )
    ...
    def draw(
        ...
        *,
        ...
        for i, line in enumerate(ChickenFarmPanel.storage_lines(farm)):
            surf = body.render(line, True, (200, 204, 214))
            surface.blit(surf, (layout.frame.left + _PANEL_PAD, details_y + i * 22))
