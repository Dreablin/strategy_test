"""Mill panel with active toggle and production details."""
...
class MillPanel:
    ...
    def supports_building(building) -> bool:
        return isinstance(building, Mill)
    ...
    def blocked_reason(mill: Mill, *, production_status: str | None) -> str:
        ...
        if status == "processing":
            return "running"
        if mill.output_amount() >= mill.output_capacity():
            return "output full"
        if mill.input_amount() <= 0:
            return "no wheat"
        return "running"
    ...
    def draw(
        ...
        *,
        ...
        progress = max(0.0, min(1.0, mill.processing_progress(now_ms)))
        if progress > 0.0:
            fill = pygame.Rect(bar_bg.left, bar_bg.top, max(1, int(round(bar_bg.width * progress))), bar_bg.height)
            pygame.draw.rect(surface, (238, 228, 192), fill, border_radius=4)
        pygame.draw.rect(surface, (116, 124, 136), bar_bg, width=1, border_radius=4)

        active_bg = (84, 112, 84) if mill.active else (92, 64, 64)
        pygame.draw.rect(surface, active_bg, layout.toggle, border_radius=6)
        label = font.render(MillPanel.toggle_label(mill), True, (240, 242, 250))
