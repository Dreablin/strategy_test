...
def test_vineyard_farm_disk_meta_and_sprite_smoke() -> None:
    """T316: meta on disk; ready + construction sprites resolve (procedural fallback)."""
    root = Path(__file__).resolve().parents[1] / "assets" / "buildings" / "vineyard_farm"
    assert (root / "asset_meta.json").is_file()

    for b_tag in ("vineyard_farm", "VINEYARD_FARM"):
        for level in (1, 5, 10):
            spr = building_sprite(b_tag, level)
            _assert_nonempty_surface(spr)
            ax, ay = building_sprite_anchor(b_tag, level)
