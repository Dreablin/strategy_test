...
def test_chicken_farm_panel_supports_building_and_toggle_click() -> None:
    ...
    layout = ChickenFarmPanel.layout(surface, farm, worker_assigned=False, production_status="No worker")
...
def test_chicken_farm_panel_blocked_reason_hints() -> None:
    farm = ChickenFarm(level=1, grid_pos=(10, 10))
    assert ChickenFarmPanel.blocked_reason(farm, worker_status="empty", production_status="No worker") == "no worker"
    ...
    assert ChickenFarmPanel.blocked_reason(farm, worker_status="assigned", production_status="Output full") == "output full"
