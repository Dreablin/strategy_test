...
def test_forester_hut_panel_buttons_do_not_overlap() -> None:
    ...
    layout = ForesterHutPanel.layout(surface, hut, worker_assigned=False)
...
def test_forester_hut_panel_click_actions_for_demolish_and_toggle() -> None:
    ...
    assert (
        ForesterHutPanel.click_action(
            surface, layout.demolish.center, hut, worker_assigned=False
        )
        == "demolish"
    )
    assert (
        ForesterHutPanel.click_action(surface, layout.toggle.center, hut, worker_assigned=False)
        == "toggle_active"
    )
