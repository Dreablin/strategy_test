...
def test_research_points_label_and_ratio() -> None:
    ...
    assert research_points_label(state) == i18n.t(
        "ui.laboratory.research_points", current=350, required=5000
    )
...
def test_panel_draws_progress_bar_fill() -> None:
    ...
    assert research_points_label(state) == i18n.t(
        "ui.laboratory.research_points", current=2500, required=5000
    )
