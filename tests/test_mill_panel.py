...
def test_mill_panel_supports_building_and_toggle_click() -> None:
    ...
    assert MillPanel.click_action(
        ...
        production_status="no_wheat",
    ) == "toggle_active"


def test_mill_panel_blocked_reason_hints() -> None:
    mill = Mill(level=1, grid_pos=(10, 10))
    assert MillPanel.blocked_reason(mill, production_status="no_worker") == "no worker"
    assert MillPanel.blocked_reason(mill, production_status="no_wheat") == "no wheat"
    ...
    assert MillPanel.blocked_reason(mill, production_status="output_full") == "output full"
