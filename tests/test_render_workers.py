...
def test_draw_workers_farmer_going_to_field_interpolates_between_tiles(monkeypatch) -> None:
    world = World()
    registry = BuildingRegistry(world)
    registry.place(TownHall, town_hall_origin_tile())
    wm = WorkerManager(registry)
    c = near_town_hall_tile()
    w = Worker("FARMER", stand_tile=c)
    w.start_move([c, (c[0] + 1, c[1])], started_ms=0, move_state="going_to_field")
    wm.add_worker(w)
