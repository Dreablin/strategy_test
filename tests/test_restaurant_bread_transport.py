...
def test_bread_tasks_generated_for_free_capacity() -> None:
    ...
    town_hall.add_to_warehouse("bread", restaurant.local_storage_capacity("bread"))
    tasks = restaurant_input_transport_tasks(registry, "bread")
    assert len(tasks) == restaurant.local_storage_capacity("bread")
...
def test_bread_tasks_skip_full_restaurant() -> None:
    world, registry, town_hall, restaurant = _setup()
    town_hall.add_to_warehouse("bread", restaurant.local_storage_capacity("bread"))
    restaurant.add_local_storage("bread", restaurant.local_storage_capacity("bread"))
    tasks = restaurant_input_transport_tasks(registry, "bread")
    assert tasks == []
