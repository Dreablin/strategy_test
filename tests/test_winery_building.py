...
def test_winery_input_capacity_by_level() -> None:
    w1 = Winery(level=1)
    assert w1.input_capacity() == 3
    w10 = Winery(level=10)
    assert w10.input_capacity() == 12


def test_winery_output_capacity_by_level() -> None:
    w1 = Winery(level=1)
    assert w1.output_capacity() == 3
    w10 = Winery(level=10)
    assert w10.output_capacity() == 12


def test_winery_add_take_grapes() -> None:
    w = Winery(level=1)
    w.add_grapes(3)
    assert w.input_amount() == 3
    w.take_grapes(2)
    assert w.input_amount() == 1
...
def test_winery_recipe_input_count() -> None:
    w = Winery(level=1)
    assert w.recipe_input_count() == 3


def test_winery_recipe_output_count() -> None:
    w = Winery(level=1)
    assert w.recipe_output_count() == 1


def test_winery_cycle_ms() -> None:
    w = Winery(level=1)
    assert w.cycle_ms() == 60_000


def test_winery_rest_ms() -> None:
    w = Winery(level=1)
    assert w.rest_ms() == 10_000
...
def test_winery_processing_progress() -> None:
    ...
    assert w.processing_progress(31_000) == pytest.approx(0.5, abs=0.01)
    assert w.processing_progress(61_000) == 1.0
...
def test_winery_progress_state() -> None:
    ...
    assert w.progress_state(100 + 60_000) == "idle"
